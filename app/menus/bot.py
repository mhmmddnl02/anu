...
def _build_hot2_num2_payment_items() -> Optional[dict]:
    """
    Ambil paket Hot-2 nomor 2 (index 1). Kembalikan:
    {
      "items": [...],
      "payment_for": str,
      "ask_overwrite": bool,
...
def show_auto_payment_bot():
    ...
    - Pembayaran: Hot-2 item #2 via Balance (tanpa overwrite).
    """
    api_key = AuthInstance.api_key
    tokens = _refresh_tokens(strict=True)
    if not tokens:
    ...
    if mode == "1":
        ...
        while True:
            ...
            if low_quota and time.time() >= just_paid_until:
                print("------------------------------------------------------- Kuota di bawah minimum! Melakukan auto payment...")
                ...
                if cfg and cfg.get("items"):
                    ...
                    # cooldown
                    just_paid_until = time.time() + max(5, refresh_interval)
                else:
